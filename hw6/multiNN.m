function [W_list,B_list,totalLossList]=multiNN(dataWithLabel, crossEntropyOn)
    ...
    if crossEntropyOn
        W_list = zeros(NUM_LAYERS, max(NUM_NODES_HID1,NUM_NODES_HID2), numFeatures, NUM_EPOCHS/5);
        B_list = zeros(NUM_LAYERS, max(NUM_NODES_HID1,NUM_NODES_HID2), NUM_EPOCHS/5);
        totalLossList = zeros(NUM_EPOCHS/5,1);        
    else
    ...
    for e=1:NUM_EPOCHS
        ...
        for i=0:numBatches-1
            ...
            for j=1:min(MINI_BATCH_SIZE, numData - i*MINI_BATCH_SIZE)
                ...
                W3_grad = W3_grad + delta3 * tanhS2'; % NUM_CLASSES by NUM_NODES_HID2
                B3_grad = B3_grad + delta3; % NUM_CLASSES by 1
                delta2 = getTanhDelta(tanhS2, W3, delta3); % NUM_NODES_HID2 by 1
                W2_grad = W2_grad + delta2 * tanhS1'; % NUM_NODES_HID2 by NUM_NODES_HID1
